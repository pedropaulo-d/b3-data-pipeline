...
**Adendo (2026-06-24, refatoração rodada 1) — secrets dummy não são mais necessários.** O contorno acima (definir `MINIO_*` dummy nos secrets do Streamlit Cloud) existia porque importar `warehouse.conexao` puxava `ingestion.config`, que valida `MINIO_*` no import. A refatoração (entrada seguinte) tornou esse import **lazy**: o dashboard não importa mais `ingestion.config`, logo o app sobe sem nenhum secret de object storage. Os secrets dummy eventualmente configurados na nuvem ficam **inertes** (ignorados) — podem ser removidos, mas não atrapalham.

---

## 2026-06-24 — Refatoração (rodada 1) — Import lazy de S3 (Desenho A) e migração da sintaxe de testes dbt

**Contexto.** Rodada de refatoração de qualidade pós-Etapa 7 (sem mudar comportamento), com quatro itens fechados. Os dois estruturais:

1. **Acoplamento dashboard → `ingestion.config`** (DT-7.2). Importar `warehouse.conexao` disparava, em cadeia, o import de `ingestion.config`, que valida `MINIO_*` já no nível do módulo (`_exigir_var`). Resultado: o dashboard — que abre o DuckDB em `read_only=True` e lê **só marts locais**, sem nunca tocar S3 — exigia credenciais de object storage para subir (daí os secrets dummy no deploy). A cadeia tinha **duas pontas**: (a) `warehouse/conexao.py` importava as constantes `MINIO_*` no topo; (b) `warehouse/__init__.py` importava `warehouse.setup` (que também importa `ingestion.config` no topo) só para reexportar `criar_schema_raw`.
2. **`RuntimeWarning` do `runpy`** (item 4) em `python -m warehouse.setup`: *"'warehouse.setup' found in sys.modules after import of package 'warehouse'"*. Mesma causa-raiz que (b): ao rodar `-m`, o `runpy` importa o pacote `warehouse` (executa o `__init__`), que importava `warehouse.setup` — deixando o módulo em `sys.modules` **antes** de o `runpy` executá-lo como `__main__`.

**Decisão.**
- **Import lazy de S3 (Desenho A).** Mover o `from ingestion.config import MINIO_*` do topo de `warehouse/conexao.py` para **dentro** de `configurar_s3()`. `obter_conexao(read_only=...)` passa a depender só do caminho do `.duckdb` (já resolvido por `Path(__file__)`), sem importar `ingestion.config`. Só quem de fato chama `configurar_s3` (setup do warehouse, `checar_warehouse`, notebook da Etapa 3) lê as credenciais — no momento da chamada.
- **`__init__` deixa de reexportar `criar_schema_raw`.** Removido `from warehouse.setup import criar_schema_raw` de `warehouse/__init__.py` (a função continua disponível em `warehouse.setup`, importada de lá pelos consumidores). Isso corta a outra ponta do acoplamento **e** elimina a pré-população de `sys.modules` que causava o `RuntimeWarning` — uma mudança resolve item 1 e item 4.
- **Migração da sintaxe de testes genéricos do dbt** (DT-6.1). Os 11 testes genéricos com argumentos (`relationships`, `accepted_values`, `dbt_utils.unique_combination_of_columns`) passaram a aninhar os argumentos sob a propriedade `arguments:`, exigida a partir do dbt 1.10 (`MissingArgumentsPropertyInGenericTestDeprecation`). `not_null`/`unique` (sem argumentos) ficam como estão.

**Racional.** O dashboard não deve depender de configuração de *ingestão* — abrir um DuckDB em read-only é uma necessidade de **caminho de arquivo**, não de credencial de S3. O import lazy é a mudança de menor superfície que desacopla sem mexer em `ingestion.config` (que segue validando `MINIO_*` para quem realmente usa S3). Reexportar `criar_schema_raw` no `__init__` nunca teve consumidor (ninguém importa de `warehouse` direto) e custava o acoplamento + o warning — removê-lo é ganho líquido. A migração dbt antecipa o ponto em que o warning vira erro, sem mudança funcional.

**Validação.** `obter_conexao` + query de marts em read-only roda com `ingestion.config` **ausente** de `sys.modules` (provado por asserção); `dashboard.data` importa limpo idem. `configurar_s3` num DuckDB em memória aplica `s3_endpoint`/`s3_access_key_id` corretamente (caminho S3 intacto para setup/`checar_warehouse`). `import warehouse` não traz mais `warehouse.setup`; `python -m warehouse.setup` com `-W error::RuntimeWarning` não dispara o warning. `dbt parse` sem o deprecation; `dbt test --select path:models/marts` → 34 PASS (inclui os `relationships`/`unique_combination` migrados); os 5 ERROR restantes são `fato_dividendos` (view → MinIO fora do ar nesta máquina), não falha de sintaxe. A suíte completa de 49 testes (`dbt build`) exige o MinIO no ar — roda na máquina do Pedro com o Docker ativo.
...
## 2026-06-25 — Refatoração (rodada 2) — Limpeza cirúrgica + itens mantidos por design

**Contexto.** Segunda rodada de refatoração de qualidade (pós-Etapa 7), guiada por uma auditoria de consistência estrutural. O veredito da auditoria foi que o código já está coeso — então esta rodada é deliberadamente pequena: aplica só os poucos achados de baixo risco e **registra** os que foram avaliados e mantidos de propósito (para não reabrir o debate em rodadas futuras).

**Aplicado (baixo risco — strings, docs e imports; sem mudança de comportamento).**
- **A1 — Referências obsoletas a `docker-compose.minio.yml`.** O arquivo foi renomeado para `docker-compose.yml` na Etapa 5, mas 8 locais ativos ainda instruíam `docker compose -f docker-compose.minio.yml up -d` (comando quebrado). Padronizados para `docker compose up -d minio mc-init` (subir só o MinIO, como o subpacote `dividendos/` já fazia): `ingestion/main.py`, `ingestion/storage.py`, `warehouse/setup.py`, `scripts/validar_idempotencia.py` + os READMEs de `ingestion/`, `warehouse/`, `scripts/` e `dbt/` (este mantém o `-f ../docker-compose.yml` por rodar a partir de `dbt/`). As menções em `docs/decisoes.md` e `CLAUDE.md` foram **preservadas** — lá a citação é histórica (descreve o próprio rename).
- **A2 — Arquivo órfão removido.** `scripts/scrips.txt` (cópia morta e desatualizada de `scripts/load_env.ps1`, nome digitado errado, não referenciado por nada — confirmado por `grep`).
- **C3 — Docstrings desatualizadas.** `ingestion/__init__.py` (dizia persistir em `data/raw/` local; hoje o raw vive no MinIO), `warehouse/__init__.py` (passou a mencionar `raw.dividendos` além de `raw.cotacoes`), `scripts/__init__.py` (estava vazio; ganhou docstring de propósito do pacote).
- **B4 — Formato de logging.** `warehouse/setup.py` alinhado ao dos demais entry points (`"%(asctime)s [%(levelname)s] %(name)s: %(message)s"` + `datefmt`).
- **B2 — Import consolidado.** `scripts/validar_etapa6.py` passou a importar `obter_conexao` no topo (junto de `CAMINHO_WAREHOUSE`), espelhando o gêmeo `checar_warehouse.py`; o import dentro de `main()` (sem ganho — o módulo já vinha do topo) foi removido.

**Mantido por design (NÃO refatorado — decisão consciente).**
- **B1 — Duplicação de `criar_cliente_s3`/`_chave_para` em `scripts/validar_idempotencia.py`.** O script reimplementa o cliente boto3 e a montagem da chave em vez de importar de `ingestion.s3_client`/`ingestion.storage`. É **deliberado**: um validador de idempotência deve ser independente do código que valida — se importasse a mesma função, um bug compartilhado (ex.: chave montada errada) passaria despercebido, pois os dois lados errariam igual. A docstring da função no script já registra isso. Acoplar para "DRY" enfraqueceria o propósito do teste.
- **B3 — `print` (não `logging`) nos scripts de relatório.** `checar_warehouse`, `validar_etapa6`, `validar_idempotencia` e o `_diagnostico` de `warehouse/setup.py` usam `print`. A saída desses utilitários **é o produto** (relatório para humano no terminal), não evento de log operacional — distinção diferente do pipeline (`ingestion/`, `warehouse/conexao.py`), que usa `logging`. O split é coerente, não descuido; a regra "nunca `print` em produção" do `CLAUDE.md` mira o código de pipeline, não os scripts de relatório.
- **C1 — Janelas repetidas em `mart_indicadores_diarios.sql`.** As médias móveis (7/30/90/200), contagens de pregões e volatilidades (30/90/252) são colunas `OVER (... ROWS BETWEEN N PRECEDING ...)` repetidas com `N` variável — geráveis por um loop Jinja. Mantidas **explícitas** de propósito: o objetivo do projeto é código legível e defensável em entrevista, e SQL explícito ("é uma `AVG` de janela 30") lê melhor que um `{% for n in [...] %}` que esconde a lógica. DRY aqui custaria clareza sem ganho real.

**Racional.** Numa peça de portfólio o critério de refatoração é "vale o risco de mexer em código que funciona?". Os itens aplicados são strings/docs/imports (risco ~nulo, alta visibilidade — instruções quebradas e docstrings mentindo saltam aos olhos de um revisor). Os mantidos têm um motivo técnico ativo (independência do validador, relatório vs. log, legibilidade) que uma refatoração "por consistência" pioraria — registrá-los aqui evita que uma rodada futura os "conserte" sem perceber o trade-off.

**Validação.** Mudanças sem efeito de runtime além de forma de log e local de import. Conferido que `python -m warehouse.setup` (com `.env` + MinIO), `python -m scripts.validar_etapa6` e `python -m scripts.checar_warehouse` seguem rodando idênticos (só muda a aparência do log e a posição dos imports).

**Escopo.** Travado em A1/A2/C3/B4/B2. Itens C2 (boilerplate `_configurar_logging`/`_parse_data` duplicado) e C4 (`dbt/.user.yml` versionado) ficaram de fora por irrelevância de impacto; B1/B3/C1 ficaram por design (acima).
